...
class Win32Window {
 public:
  ...
  // Set minimum and maximum window sizes in logical pixels (device-independent).
  void SetMinimumSize(const Size& size);
  void SetMaximumSize(const Size& size);
  // Convenience: set fixed size (min == max).
 ...
 private:
  ...
  // Min/max physical pixel sizes enforced in WM_GETMINMAXINFO (0 = no limit).
